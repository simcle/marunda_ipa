...
const pollData = async () => {
    ...
    try {
        const message = {}
        const test = await client.readHoldingRegisters(3, 2)
        const wordBlo = test.data[0];
        const wordPmp = test.data[1];
        
        const bitBlo = (wordBlo >> 15) & 1
        const bitpmp = (wordPmp >> 0) & 1
        
        console.log(bitBlo, bitpmp)
